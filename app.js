...
let SERVE_IMAGES_LOADED = false;
async function ensureServeImagesMap() {
  if (SERVE_IMAGES_LOADED) return SERVE_IMAGES_MAP;
  const colRef = collection(db, 'serves');
  const snapshot = await getDocs(colRef);
  snapshot.docs.forEach(doc => {
    const data = doc.data();
    if (data.name && data.image) {
      SERVE_IMAGES_MAP[data.name] = data.image;
    }
  });
  SERVE_IMAGES_LOADED = true;
  return SERVE_IMAGES_MAP;
}

async function loadServeImagesMap() {
  const colRef = collection(db, 'serves');
  const snapshot = await getDocs(colRef);
  const map = {};
  snapshot.docs.forEach(doc => {
    const data = doc.data();
    if (data.name && data.image) {
      map[data.name] = data.image; // data:image/webp;base64,...
    }
  });
  return map;
}
...
// === 모든 이미지 일괄 교체 (정확 매핑) ===
async function replaceAllServeImages() {
  await ensureServeImagesMap();
  // id/class와 Firestore name 매핑 테이블
  const idToName = {
    'summit-image1': 'SummitImage1',
    'summit-image2': 'SummitImage2',
    'summit-image3': 'SummitImage3',
    'summit-image4': 'SummitImage4',
    'summit-image5': 'SummitImage5',
    'summit-image6': 'SummitImage6',
    'summit-image7': 'SummitImage7',
    'oreumi': 'Oreumi',
    'summit-main-logo': 'SummitMainLogo1',
    'summit-serve-logo': 'SummitServeLogo1',
  };
  Object.entries(idToName).forEach(([id, name]) => {
    const el = document.getElementById(id);
    if (el && SERVE_IMAGES_MAP[name]) {
      el.src = SERVE_IMAGES_MAP[name];
    }
  });
  // class로만 지정된 이미지
  const classToName = {
    'center-logo': 'SummitServeLogo1',
    'mascot-image': 'Oreumi',
    'result-top-left-logo': 'SummitMainLogo1',
    'result-logo': 'SummitServeLogo1',
  };
  Object.entries(classToName).forEach(([cls, name]) => {
  ...
  // inform-deck value-card (img에 직접 class와 id)
  document.querySelectorAll('.inform-deck .value-card').forEach(img => {
    const id = img.id;
    if (id && idToName[id] && SERVE_IMAGES_MAP[idToName[id]]) {
      img.src = SERVE_IMAGES_MAP[idToName[id]];
    }
  });
}

document.addEventListener('DOMContentLoaded', () => {
  replaceAllServeImages();
});
...
function addProjectCard({ name, year, desc, images }) {
  ...
  // 박스 썸네일 이미지 (Base64로 대체, 정확 매핑)
  if (images && images.length > 0) {
    const photoDiv = node.querySelector('.project-photo');
    if (photoDiv) {
      const img = document.createElement('img');
      // images[0]이 SummitImage1 또는 SummitImage1.webp 등일 수 있음
      let imgName = images[0];
      imgName = imgName.replace(/\.(webp|png|jpg)$/i, '');
      if (SERVE_IMAGES_MAP[imgName]) {
        img.src = SERVE_IMAGES_MAP[imgName];
      } else {
        img.src = images[0];
      }
      img.alt = name;
  ...
  if (cardElem) {
    cardElem.addEventListener('click', function () {
      ...
      // === 뒤로가기 버튼 동적 생성 및 배치 ===
      let backBtn = document.getElementById('project-detail-back-btn');
      if (!backBtn) {
        ...
        backBtn.addEventListener('click', (e) => {
          e.preventDefault();
          // 세부 탭 스크롤 위치 저장
          if (!window.__detailScrollY) window.__detailScrollY = 0;
          window.__detailScrollY = window.scrollY || window.pageYOffset || 0;
          detailSection.style.transition = 'opacity 0.4s';
          detailSection.style.opacity = '0';
          setTimeout(() => {
            detailSection.style.display = 'none';
            // body에서 detail mode 클래스 제거
            document.body.classList.remove('project-detail-mode');
            // 프로젝트 탭 높이 복원
            ...
            setTimeout(() => {
              ...
              if (window.__projectScrollY) window.scrollTo(0, window.__projectScrollY);
            }, 10);
          }, 400);
        });
      }
      // 버튼을 project-detail-box 바로 위에, 왼쪽 정렬로 배치
      // 기존 controls 영역도 유지
      const controls = document.querySelector('.project-detail-controls');
      ...
      function renderImage(direction = null) {
        if (!detailBox) return;
        // 기존 이미지가 있으면 애니메이션 적용
        const prevImg = detailBox.querySelector('img.project-image');
        if (images && images.length > 0) {
          const img = document.createElement('img');
          ...
          if (prevImg) {
            if (direction === 'left') {
              ...
              prevImg.classList.add('slide-out-right');
            } else if (direction === 'right') {
              prevImg.classList.remove('active');
            ...
            // 애니메이션 끝나면 제거
            prevImg.addEventListener('transitionend', () => {
              if (prevImg.parentNode) prevImg.parentNode.removeChild(prevImg);
              // 이미지가 완전히 바뀐 뒤 높이 재조정
              updateDetailScrollPadding();
            }, { once: true });
          }
          // 새 이미지가 들어오면 active로 변경
          setTimeout(() => {
      ...
      // 세부 탭 페이드 인/아웃
      // 프로젝트 탭 스크롤 위치 저장
      if (!window.__projectScrollY) window.__projectScrollY = 0;
      window.__projectScrollY = window.scrollY || window.pageYOffset || 0;
      ...
      // body에 detail mode 클래스 추가
      document.body.classList.add('project-detail-mode');
      setTimeout(() => {
        projectSection.style.opacity = '0';
        setTimeout(() => {
          projectSection.style.display = 'none';
          detailSection.style.opacity = '1';
          // 세부 탭 컨텐츠 높이에 맞게 min-height 조정
          updateDetailScrollPadding();
          // 세부 탭 진입 시 스크롤 맨 위로 이동
          window.scrollTo(0, 0);
        }, 400);
      }, 10);
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (backBtn) {
    backBtn.addEventListener('click', (e) => {
      ...
      if (!window.__detailScrollY) window.__detailScrollY = 0;
      window.__detailScrollY = window.scrollY || window.pageYOffset || 0;
      // 세부 탭 닫고 프로젝트 목록으로 복귀
      const detailSection = document.getElementById('project-detail-section');
      const projectSection = document.getElementById('project-section');
      if (detailSection && projectSection) {
        detailSection.style.transition = 'opacity 0.4s';
        detailSection.style.opacity = '0';
        setTimeout(() => {
          detailSection.style.display = 'none';
  ...
  useData.forEach((m) => {
    ...
    if (imgEl) {
      if (m.image) {
        ...
        imgEl.src = SERVE_IMAGES_MAP['SummitMainLogo1'] || '';
        imgEl.alt = 'No Image';
      }
    }
    // 이름, 자기소개
...
// ===== 모든 .result-top-left-logo 클릭 시 홈 이동 (id/class robust) =====
let __resultLogoBound = false;
function setupResultHomeLogo() {
  if (__resultLogoBound) return;
  // 홈 섹션의 .top-left-logo는 제외, 결과/합격자/불합격자 섹션의 .result-top-left-logo만
  const homeLogo = document.querySelector('.top-left-logo');
  // 중복 id 방지: class 기반으로 모두 선택
  const logos = Array.from(document.querySelectorAll('img.result-top-left-logo')).filter(el => el !== homeLogo);
